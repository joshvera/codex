...
#[tokio::test]
async fn plan_mode_model_reads_top_level_config() -> anyhow::Result<()> {
    let cfg: ConfigToml = toml::from_str(
        r#"
model = "gpt-5.3-codex"
...
#[tokio::test]
async fn plan_mode_model_uses_profile_precedence() -> anyhow::Result<()> {
    let codex_home = TempDir::new()?;
    let selected_config = codex_home.path().join("deep.config.toml");
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        r#"
model = "gpt-5.3-codex"
...
"#,
    ...
    std::fs::write(&selected_config, r#"plan_mode_model = "gpt-5.4""#)?;
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .loader_overrides(LoaderOverrides {
            user_config_path: Some(selected_config.abs()),
            user_config_profile: Some("deep".parse().expect("profile-v2 name")),
            ..LoaderOverrides::without_managed_config_for_tests()
        })
        .build()
        .await?;
