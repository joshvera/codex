...
impl ChatWidget {
    ...
    fn model_selection_actions(
        ...
        vec![Box::new(move |tx| {
            ...
            } else if scope_changes.requires_prompt() {
    ...
    pub(crate) fn open_plan_selection_scope_prompt(
        ...
        let selection_label =
            Self::plan_mode_selection_label(&model, effort.clone(), scope_changes);
        let plan_only_description = format!("Always use {selection_label} in Plan mode.");
        let all_modes_description =
            Self::plan_mode_scope_all_modes_description(&model, effort.clone(), scope_changes);
        let subtitle =
            Self::plan_mode_selection_scope_subtitle(&model, effort.clone(), scope_changes);
        let warning = effort
            .as_ref()
            .and_then(|effort| self.ultra_reasoning_concurrency_warning(effort));
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            title: Some(PLAN_MODE_SELECTION_SCOPE_TITLE.to_string()),
            subtitle: Some(subtitle),
            footer_hint: Some(standard_popup_hint_line()),
            items: vec![
                SelectionItem {
                    name: PLAN_MODE_SELECTION_SCOPE_PLAN_ONLY.to_string(),
                ...
                SelectionItem {
                    name: PLAN_MODE_SELECTION_SCOPE_ALL_MODES.to_string(),
        ...
        self.notify(Notification::PlanModePrompt {
            title: PLAN_MODE_SELECTION_SCOPE_TITLE.to_string(),
        });
    }
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        if choices.len() == 1 && advanced_choices.is_empty() {
            ...
            let scope_changes =
                self.plan_mode_selection_scope_changes(&selected_model, selected_effort.clone());
            if scope_changes.requires_prompt() {
                self.app_event_tx
        ...
        for choice in choices.iter() {
            ...
            let scope_changes =
                self.plan_mode_selection_scope_changes(model_slug.as_str(), choice_effort.clone());
            let actions =
                self.model_selection_actions(model_slug.clone(), choice_effort, scope_changes);
    ...
    pub(crate) fn open_advanced_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        for effort in choices {
            ...
            let scope_changes =
                self.plan_mode_selection_scope_changes(model_slug.as_str(), Some(effort.clone()));
